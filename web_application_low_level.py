...
URLS = {
    '/': 'hello index', # Для главной страницы будет ответ hello index
    '/blog': 'hello blog',
}
...
def generate_response(request):
    method, url = parse_request(request) # Распарсим наш реквест, получим http метод и url запрос.
...
def run():
    ...
    while True: # так как мы не знаем сколько длится сессия делаем ее зацикленной.
        # медот accept() возвращает то что он получил от клиента. (передает кортеж из двух элементов сокета и адресса)
